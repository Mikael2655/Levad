...
export default async function DashboardPage() {
  ...
  const [matches, predictions, bonusQuestions, bonusAnswers, leaderboard] = await Promise.all([
    ...
    prisma.bonusQuestion.findMany({ orderBy: { createdAt: 'desc' } }),
  ...
  const now = new Date()
  const upcoming = matches.filter((m) => m.status !== 'FINISHED' && new Date(m.kickoff) > now)
  const past = matches.filter((m) => m.status === 'FINISHED' || new Date(m.kickoff) <= now)
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-10 space-y-10">
      ...
      {past.length > 0 && (
        <section>
          <h2 className="text-xl font-bold mb-4">Matchs joués</h2>
          <div className="space-y-3">
            {past.map((match) => (
              <MatchPredictionCard
                key={match.id}
